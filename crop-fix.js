...
(function(){
  ...
  function draw(){
    ...
    ctx.fillRect(0,0,c.width,crop.y);ctx.fillRect(0,crop.y,crop.x,crop.h);
    ctx.fillRect(crop.x+crop.w,crop.y,c.width-crop.x-crop.w,crop.h);ctx.fillRect(0,crop.y+crop.h,c.width,c.height-crop.y-crop.h);
    ...
    for(const [x,y] of [[crop.x,crop.y],[crop.x+crop.w,crop.y],[crop.x,crop.y+crop.h],[crop.x+crop.w,crop.y+crop.h]]){
      ctx.beginPath();ctx.arc(x,y,hs/2,0,Math.PI*2);ctx.fillStyle='#2563eb';ctx.fill();ctx.strokeStyle='#fff';ctx.lineWidth=3;ctx.stroke();
    }
  }

  function initialCrop(c){const pad=Math.round(Math.min(c.width,c.height)*.10);crop={x:pad,y:pad,w:c.width-pad*2,h:c.height-pad*2}}

  function buildColorMask(data,w,h){
    const mask=new Uint8Array(w*h);
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){
      const i=(y*w+x)*4,r=data[i],g=data[i+1],b=data[i+2];
      const mx=Math.max(r,g,b),mn=Math.min(r,g,b),sat=mx?((mx-mn)/mx)*255:0;
      // Do not use RGB distance alone: the beige app background is close to
      // the pink/yellow palette but has much lower saturation.
  ...
  function dilate(mask,w,h,r){
    ...
    for(let y=0;y<h;y++){
      ...
      for(let x=0;x<w+r;x++){
        ...
        const rem=x-span;if(rem>=0)sum-=mask[y*w+rem];
        const out=x-r;if(out>=0&&out<w)a[y*w+out]=sum>0?1:0;
      }
    }
    for(let x=0;x<w;x++){
      let sum=0;
      for(let y=0;y<h+r;y++){
        if(y<h)sum+=a[y*w+x];
        const rem=y-span;if(rem>=0)sum-=a[rem*w+x];
        const out=y-r;if(out>=0&&out<h)b[out*w+x]=sum>0?1:0;
      }
    }
    return b;
  }
  ...
  function findOuterGrid(c){
    ...
    const data=tctx.getImageData(0,0,w,h).data,mask=buildColorMask(data,w,h);
    ...
    for(let sy=0;sy<h;sy++)for(let sx=0;sx<w;sx++){
      const start=sy*w+sx;if(!blob[start]||seen[start])continue;
      ...
      // Scale-aware limits are important on tall phone screenshots. The old
      // fixed 120px minimum rejected the 8x8 board after downscaling.
      const minRequired=Math.max(55,Math.min(w,h)*.18);
      if(minSide<minRequired||ratio<.70||ratio>1.30||fill<.40)continue;
      const square=1-Math.abs(1-ratio),score=area*square*fill;
  ...
  function pointerMove(e){
    if(!drag)return;e.preventDefault();const c=getCanvas(),p=point(e),o=drag.orig,MIN=Math.max(60,Math.min(c.width,c.height)*.12);
  ...
  function showCrop(image){
    img=image;const c=$('previewCanvas');
    const maxW=Math.min(1100,window.innerWidth*.92),maxH=Math.min(620,window.innerHeight*.62),scale=Math.min(1,maxW/img.width,maxH/img.height);
    ...
    const help=document.createElement('div');help.className='crop-help';help.textContent='🤖 The main puzzle grid is detected automatically. Drag the blue corners to fine-tune it.';preview.insertBefore(help,wrap.nextSibling);
    let actions=preview.querySelector('.crop-actions');if(actions)actions.remove();actions=document.createElement('div');actions.className='crop-actions';
    actions.innerHTML='<button id="autoCrop" class="crop-auto">🎯 Auto Grid</button><button id="resetCrop">↺ Reset</button><button id="cropDetect" class="crop-primary">✂️ Crop & Detect</button><button id="cancelCrop">Cancel</button>';preview.appendChild(actions);

    const ok=autoDetectOuterGrid();if(!ok){initialCrop(cc);$('status').textContent='Screenshot loaded. Auto Grid could not confidently find the board. Adjust the crop manually, or tap Auto Grid again.'}
    draw();
    ...
    $('autoCrop').onclick=()=>{if(!autoDetectOuterGrid())$('status').textContent='⚠️ Auto Grid could not find the main puzzle board. Please crop it manually.'};
    $('resetCrop').onclick=()=>{initialCrop(cc);draw();$('status').textContent='Crop reset. Adjust the blue corners, then tap Crop & Detect.'};
    $('cancelCrop').onclick=()=>{preview.hidden=true};$('cropDetect').onclick=()=>cropAndDetect();preview.hidden=false;
  }

  function cropAndDetect(){
    if(!img||!crop)return;const src=getCanvas(),out=$('previewCanvas'),ctx=out.getContext('2d');
    out.width=Math.max(1,Math.round(crop.w));out.height=Math.max(1,Math.round(crop.h));ctx.clearRect(0,0,out.width,out.height);ctx.drawImage(src,crop.x,crop.y,crop.w,crop.h,0,0,out.width,out.height);out.hidden=false;
    const wrap=preview.querySelector('.crop-wrap');if(wrap)wrap.remove();const help=preview.querySelector('.crop-help');if(help)help.remove();const actions=preview.querySelector('.crop-actions');if(actions)actions.remove();
    $('status').textContent='Cropped screenshot ready. Detecting board...';if(typeof window.detectBoard==='function')window.detectBoard();
  }
