...
(function(){
  ...
  function autoDetectCrop(c){
    ...
    let rows=getRuns(rowScore,Math.max(12,Math.floor(w*.18)),5),bestRows=[];
    for(let i=0;i<rows.length;i++){const group=[rows[i]];for(let j=i+1;j<rows.length;j++){const prev=group[group.length-1],cur=rows[j],gap=cur[0]-prev[1]-1,ph=prev[1]-prev[0]+1,ch=cur[1]-cur[0]+1;if(gap>12||ch<ph*.45||ch>ph*1.8)break;group.push(cur)}if(group.length>bestRows.length)bestRows=group}
    if(bestRows.length<4||bestRows.length>12)return false;
    const y0=bestRows[0][0],y1=bestRows[bestRows.length-1][1],boardH=y1-y0+1,colScore=new Int32Array(w);
    for(let x=0;x<w;x++){let s=0;for(let y=y0;y<=y1;y++)s+=mask[y*w+x];colScore[x]=s}
    const cols=getRuns(colScore,Math.max(12,Math.floor(boardH*.18)),5);if(cols.length!==bestRows.length)return false;
    const widths=cols.map(v=>v[1]-v[0]+1),heights=bestRows.map(v=>v[1]-v[0]+1),med=a=>{const q=a.slice().sort((x,y)=>x-y);return q[Math.floor(q.length/2)]},mw=med(widths),mh=med(heights);
    if(widths.some(v=>Math.abs(v-mw)>mw*.45)||heights.some(v=>Math.abs(v-mh)>mh*.45))return false;
    const x0=cols[0][0],x1=cols[cols.length-1][1],pad=Math.max(4,Math.round(Math.min(mw,mh)*.10));
    crop={x:clamp(x0-pad,0,w-1),y:clamp(y0-pad,0,h-1),w:clamp(x1-x0+1+pad*2,1,w),h:clamp(y1-y0+1+pad*2,1,h)};return true;
  }
  function initialCrop(c){if(!autoDetectCrop(c)){const padX=Math.round(c.width*.025),padY=Math.round(c.height*.025);crop={x:padX,y:padY,w:c.width-padX*2,h:c.height-padY*2}}}
